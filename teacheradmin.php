...
if (!isset($_SESSION['user_id'])) {
    header("Location: signIn.php?type=danger&message=Please Login First!!!");
    exit;
}
...
$teacherIdInt = (int) $_SESSION["user_id"];

$stmt->bind_param("i", $teacherIdInt);
...
<body>
    ...
    <div class="course-container">
        <div class="course-header">
            ...
            <a href="courseCreate.php"><img src="utils/icons/plus-solid-full.svg" alt="add course" class="icon"
                    id="plus-icon"></a>
        </div>
            ...
            <?php while ($course = $courses->fetch_assoc()): ?>
                <div class="course">
                    <div class="course-left">
                        <img src="<?= htmlspecialchars($course['img']) ?>" alt="course image">
                    </div>
                    <div class="course-right">
                        <a href="teacherViewCourse.php?courseid=<?= htmlspecialchars($course['id']) ?>">
                            <h2 class="course-title"><?= htmlspecialchars($course['title']) ?></h2>
                        </a>
                        <h3>Description:</h3>
                        <p class="indent"><?= htmlspecialchars($course['desc']) ?></p>
                        <div class="mt-5"><img src="utils/icons/category-sold-full.svg" alt="category"
                                class="icon"><?= htmlspecialchars($course['categoryName']) ?></div>
                        ...
                        <span class="teacher"><img src="utils/icons/chalkboard-user-solid-full.svg" alt="teacher-icon"
                                class="icon"><?= $_SESSION["username"] ?></span>
